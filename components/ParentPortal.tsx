...
import { Student, AttendanceRecord, PerformanceRecord, AttendanceStatus, BehaviorStatus, MessageLog, SystemUser, Exam } from '../types';
import { getMessages, getExams } from '../services/storageService';
import { User, Calendar, Award, LogOut, Phone, Mail, ChevronDown, CheckCircle, AlertTriangle, Clock, X, MessageSquare, TrendingUp, ShieldCheck, ChevronLeft, ChevronRight, Bell, FileQuestion } from 'lucide-react';
...
const ParentPortal: React.FC<ParentPortalProps> = ({ parentPhone, allStudents, attendance, performance, onLogout }) => {
    ...
    const [activeTab, setActiveTab] = useState<'OVERVIEW' | 'MESSAGES' | 'CALENDAR'>('OVERVIEW');
        ...
        <div className="min-h-screen bg-gray-100 font-sans text-right" dir="rtl">
            ...
            <div className="bg-white border-b sticky top-[110px] z-40 shadow-sm">
                <div className="max-w-6xl mx-auto flex">
                    <button onClick={() => setActiveTab('OVERVIEW')} className={`flex-1 py-3 text-sm font-bold border-b-2 ${activeTab === 'OVERVIEW' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500'}`}>نظرة عامة</button>
                    <button onClick={() => setActiveTab('MESSAGES')} className={`flex-1 py-3 text-sm font-bold border-b-2 ${activeTab === 'MESSAGES' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500'}`}>
                        الرسائل {messages.length > 0 && <span className="bg-red-500 text-white text-[10px] px-1.5 rounded-full ml-1">{messages.length}</span>}
                    </button>
                    <button onClick={() => setActiveTab('CALENDAR')} className={`flex-1 py-3 text-sm font-bold border-b-2 ${activeTab === 'CALENDAR' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500'}`}>التقويم</button>
                </div>
            </div>
