        ...
        <div className="flex-1 bg-white rounded-[2.5rem] border shadow-sm flex flex-col overflow-hidden animate-fade-in">
            ...
            <div className="p-5 border-b bg-slate-50/50 flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="flex flex-wrap gap-3 w-full md:w-auto">
                    <div className="relative flex-1 md:w-64">
                        ...
                        <input 
                            className="w-full pr-10 pl-4 py-2 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500" 
                    ...
                    <select 
                        className="p-2 border border-slate-200 rounded-xl text-xs font-black bg-white outline-none"
                        value={historyClass}
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
                <table className="w-full text-right text-xs">
                    <thead className="bg-slate-50/80 sticky top-0 z-10 border-b text-slate-500">
                        ...
                        {filteredHistory.map((rec, idx) => {
                            ...
                            return (
                                <tr key={rec.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="p-4 font-bold text-slate-500 whitespace-nowrap">{rec.date}</td>
                                    <td className="p-4">
                                        <p className="font-black text-slate-800">{student?.name || 'طالب محذوف'}</p>
                                        <p className="text-[9px] text-slate-400 font-bold">{student?.className}</p>
                                    </td>
                                    <td className="p-4">
                                        <span className="font-bold text-indigo-600">{rec.subject || 'عام'}</span>
                                        {rec.period && <span className="mr-2 text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded text-[8px]">ح{rec.period}</span>}
                                    </td>
                ...
                {filteredHistory.length === 0 && (
                    <div className="p-20 text-center text-slate-300 font-black italic flex flex-col items-center gap-3">
                        <History size={48} className="opacity-10"/>
                        لا توجد سجلات حضور مخزنة تطابق هذه المعايير
                    </div>
                )}
            </div>
        </div>
