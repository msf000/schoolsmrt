...
const Attendance: React.FC<AttendanceProps> = ({ students, attendanceHistory, onSaveAttendance, currentUser }) => {
  ...
  const handleUpdateStatus = async (studentId: string, status: AttendanceStatus) => {
    ...
    try { await saveAttendance([record]); onSaveAttendance([record]); } catch (e) { showToast('فشل في حفظ السجل', 'ERROR'); }
  };
  ...
  return (
    <div className="space-y-6 page-enter font-tajawal">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 className="text-2xl font-bold text-slate-900">رصد الحضور اليومي</h1>
            <p className="text-slate-500 text-sm">تسجيل حضور وغياب الطلاب بشكل يدوي أو عبر الكاميرا.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        <div className="p-4 bg-slate-50/50 border-b border-slate-200 flex flex-wrap gap-4 items-end">
            <div className="w-full md:w-48">
                ...
                <select value={selectedClass} onChange={e => setSelectedClass(e.target.value)} className="w-full p-2 bg-white border border-slate-200 rounded-xl text-sm font-medium outline-none shadow-sm">
                    {uniqueClasses.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
            </div>
            <div className="w-full md:w-48">
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 mr-1">التاريخ</label>
                <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="w-full p-2 bg-white border border-slate-200 rounded-xl text-sm font-medium outline-none shadow-sm"/>
            </div>
            <div className="flex-1 min-w-[200px]">
                <div className="relative">
                    <Search size={16} className="absolute right-3 top-2.5 text-slate-400"/>
                    <input className="w-full pr-9 pl-4 py-2 bg-white border border-slate-200 rounded-xl text-sm outline-none shadow-sm" placeholder="بحث سريع..." value={searchTerm} onChange={e=>setSearchTerm(e.target.value)}/>
                </div>
            </div>
            <button className="px-4 py-2 bg-brand-100 text-brand-700 rounded-xl text-sm font-bold hover:bg-brand-200 transition-all flex items-center gap-2">
                <Camera size={16}/> رصد ذكي
            </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-[400px]">
            {activeTab === 'RECORD' ? (
                <table className="w-full text-right text-sm">
                    <thead>
                        <tr className="text-slate-400 font-bold border-b border-slate-100">
                            ...
                            <th className="px-6 py-4 text-center">قرار الحضور</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {filteredStudents.map((student, idx) => {
                            ...
                            return (
                                <tr key={student.id} className="hover:bg-slate-50/50 transition-colors h-16">
                ...
                <div className="p-8 text-center text-slate-400">
                    ...
                    <p>سجل الأرشيف يعرض العمليات السابقة لكل طالب بشكل مفصل.</p>
                </div>
            )}
        </div>
      </div>
...
const StatusBtn = ({ active, onClick, label, color }: any) => {
    const colors: any = {
        emerald: active ? 'bg-emerald-500 text-white border-emerald-500 shadow-md' : 'bg-white text-slate-400 border-slate-200 hover:bg-emerald-50 hover:text-emerald-600',
        rose: active ? 'bg-rose-500 text-white border-rose-500 shadow-md' : 'bg-white text-slate-400 border-slate-200 hover:bg-rose-50 hover:text-rose-600',
        amber: active ? 'bg-amber-500 text-white border-amber-500 shadow-md' : 'bg-white text-slate-400 border-slate-200 hover:bg-amber-50 hover:text-amber-600'
    };
    return (
        <button onClick={onClick} className={`px-4 py-1.5 rounded-lg text-[11px] font-bold transition-all border ${colors[color]}`}>{label}</button>
    );
};
