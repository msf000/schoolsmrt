...
import { FileText, AlertTriangle, Printer, Download, CheckCircle, TrendingUp, BarChart3, Activity, BrainCircuit, Users } from 'lucide-react';
    ...
    return (
        <div className="p-6 h-full flex flex-col bg-gray-50 animate-fade-in overflow-hidden">
            <div className="mb-6 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2"><FileText className="text-purple-600"/> مركز التقارير</h2>
                </div>
                        ...
                        {selectedClass ? (
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border shadow-sm flex flex-col">
                                    <h3 className="font-black text-gray-800 mb-8 flex items-center gap-2"><PieChart size={20} className="text-indigo-600"/> التوزيع العام لأنماط التعلم</h3>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-right text-sm">
                                <thead className="bg-gray-50 font-bold">
                                    <tr><th className="p-4">اسم الطالب</th><th className="p-4 text-center">الحضور</th><th className="p-4 text-center">المعدل</th><th className="p-4 text-center">النمط</th></tr>
                                </thead>
                                <tbody className="divide-y">
                                    {students.filter(s => s.className === selectedClass).map(s => (
                                        <tr key={s.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="p-4 text-center font-black text-indigo-600">--</td>
                                            <td className="p-4 text-center"><span className="bg-indigo-50 text-indigo-700 px-2 py-1 rounded text-[10px] font-bold">{s.learningStyle || 'لم يختبر'}</span></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
