...
const FormsAnalyzer: React.FC<Props> = ({ students, currentUserId }) => {
    ...
    const [compareRecordId, setCompareRecordId] = useState<string>(''); // السجل المراد المقارنة معه
    ...
    // منطق تقرير المقارنة (بين الاختبار المختار حالياً واختبار آخر من السجل)
    const comparisonData = useMemo(() => {
        if (!activeRecord || !compareRecordId) return [];
        const compareTarget = history.find(h => h.id === compareRecordId);
        if (!compareTarget) return [];

        return Object.entries(activeRecord.studentResponses).map(([sid, res]) => {
            const student = students.find(s => s.id === sid);
            const baseRes = compareTarget.studentResponses[sid];
            ...
            const indicator = Math.round((score2 / activeRecord.questions.length) * 100);

            const getGrade = (s: number, total: number) => {
                const p = (s / total) * 100;
            ...
            return {
                sid,
                name: student?.name || 'طالب مجهول',
                baseScore: score1,
                baseTotal: compareTarget.questions.length,
                baseGrade: getGrade(score1, compareTarget.questions.length),
                targetScore: score2,
                targetTotal: activeRecord.questions.length,
                targetGrade: getGrade(score2, activeRecord.questions.length),
                diff,
                improvementPct,
                indicator
            };
        });
    }, [activeRecord, compareRecordId, history, students]);
        ...
        <div className="p-6 h-full flex flex-col bg-gray-50 animate-fade-in overflow-hidden">
            ...
            {activeRecord && stats ? (
                <div className="flex-1 overflow-hidden flex flex-col gap-4 animate-slide-up">
                    ...
                    <div className="bg-white p-4 rounded-2xl border shadow-sm flex flex-col md:flex-row justify-between items-center print:hidden gap-4">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        {detailTab === 'COMPARISON' ? (
                            /* --- تقرير مقارنة الاختبارات (التصميم المرفق في الصورة) --- */
                            <div className="max-w-[210mm] mx-auto bg-white p-6 shadow-2xl overflow-x-auto print:shadow-none print:border-none">
                                ...
                                {/* اختيار اختبار المقارنة (يظهر فقط في المتصفح) */}
                                <div className="mb-6 p-4 bg-blue-50 rounded-xl border-2 border-blue-200 print:hidden flex items-center gap-4">
                                    ...
                                    >
                                        ...
                                        {history.filter(h => h.id !== activeRecord.id && h.className === activeRecord.className).map(h => (
                                ...
                                {compareRecordId ? (
                                    <div className="border-2 border-[#003366]">
                                        {/* شريط معلومات الكشف العلوي */}
                                        <div className="bg-[#003366] text-white p-2.5 flex items-center">
                                            <div className="flex-1 font-black text-center text-sm">كشف رصد درجات الاختبارات المدرسية</div>
                                            <div className="w-1/4 font-bold text-center text-xs border-r border-white/30">علوم الأرض والفضاء</div>
                                        </div>

                                        {/* بيانات الصف */}
                                        <div className="grid grid-cols-5 text-[10px] font-black border-b border-[#003366] text-center bg-gray-50 h-8 items-center">
                                            <div className="border-l border-[#003366]">العام الدراسي: {headerConfig?.academicYear}</div>
                                            <div className="border-l border-[#003366]">الفصل الدراسي: {headerConfig?.term}</div>
                                            <div className="border-l border-[#003366]">الفصل: {activeRecord.className.split(' ').pop()}</div>
                                            <div className="border-l border-[#003366]">الصف: {activeRecord.className.replace(/\d+/g, '').trim()}</div>
                                            <div>المادة</div>
                                        </div>
                                        ...
                                        <table className="w-full border-collapse text-[10px] text-center table-fixed">
                                            <thead className="bg-gray-100 font-black">
                                                <tr className="h-10">
                                                    ...
                                                    <th colSpan={2} className="border border-[#003366] bg-gray-200">اختبار تشخيصي</th>
                                                    <th colSpan={2} className="border border-[#003366]">نسبة مكتسبات المتعلم وفق</th>
                                                    <th rowSpan={2} className="border border-[#003366] w-12 bg-white">مقدار التغير</th>
                                                    <th rowSpan={2} className="border border-[#003366] w-12 bg-white">نسبة التحسين</th>
                                                    <th rowSpan={2} className="border border-[#003366] w-24 bg-white">مؤشر التحصيل الدراسي</th>
                                                </tr>
                                                <tr className="h-10">
                                                    <th className="border border-[#003366] bg-gray-50">الدرجة المكتسبة</th>
                                                    <th className="border border-[#003366] bg-gray-50">التقدير</th>
                                                    <th className="border border-[#003366] bg-gray-50 font-bold">اختبار تشخيصي</th>
                                                    <th className="border border-[#003366] bg-gray-50 font-bold">0</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {comparisonData.map((row, idx) => (
                                                    <tr key={row.sid} className="h-9 hover:bg-gray-50 border-b border-[#003366]">
                                                        ...
                                                        <td className="border border-[#003366] font-mono bg-blue-50/20">{row.baseScore > 0 ? ((row.baseScore / row.baseTotal) * 100).toFixed(2) : ''}</td>
                                                        <td className="border border-[#003366] font-mono"></td>
                                                        <td className={`border border-[#003366] font-black ${row.diff > 0 ? 'text-green-600' : 'text-red-500'}`}>{row.diff}</td>
                                                        <td className="border border-[#003366] font-mono">{row.improvementPct}%</td>
                                                        <td className="border border-[#003366] p-1.5">
                                                            <div className="flex flex-col gap-0.5">
                                                                ...
                                                                <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden border border-black/5">
                                                                    <div 
                                                                        className={`h-full ${row.indicator >= 90 ? 'bg-green-500' : row.indicator >= 70 ? 'bg-blue-500' : 'bg-orange-400'}`}
                                                                        style={{width: `${row.indicator}%`}}
                                                                    ></div>
                                                                </div>
                                                            </div>
                                                        </td>
                                                ...
                                                {/* صفوف فارغة لتكملة الشكل */}
                                                {Array.from({ length: Math.max(0, 15 - comparisonData.length) }).map((_, i) => (
                                                    <tr key={`empty-${i}`} className="h-9 border-b border-[#003366]">
                                                        <td className="border border-[#003366] bg-gray-100/50">{comparisonData.length + i + 1}</td>
                                                        <td className="border border-[#003366]"></td><td className="border border-[#003366]"></td><td className="border border-[#003366]"></td><td className="border border-[#003366]"></td><td className="border border-[#003366]"></td><td className="border border-[#003366]"></td><td className="border border-[#003366]"></td><td className="border border-[#003366]"></td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                    ...
                                    <div className="py-20 text-center flex flex-col items-center gap-4 text-gray-400">
                                        ...
                                        <p className="font-bold">يرجى اختيار اختبار المقارنة (القبلي أو التشخيصي) من القائمة المنسدلة أعلاه لتوليد التقرير.</p>
                                    </div>
                                )}
                            ...
                            /* --- كشف رصد المهارات (النموذج الأخضر) --- */
                                ...
                                <div className="border-2 border-[#003366] overflow-hidden">
                                    ...
                                    <table className="w-full border-collapse border-b-2 border-[#003366] text-[10px] text-center font-bold mb-8">
                                        <thead className="bg-blue-50">
                                            <tr><th className="border-2 border-[#003366] p-2 w-12">م</th><th className="border-2 border-[#003366] p-2">المهارة المستهدفة</th><th className="border-2 border-[#003366] p-2 w-32">نسبة الإتقان</th></tr>
                                        </thead>
                                        <tbody>
                                            {activeRecord.questions.map((q, idx) => (
                                                <tr key={idx} className="h-8"><td className="border-2 border-[#003366]">{idx + 1}</td><td className="border-2 border-[#003366] text-right pr-3">{q.learningOutcome}</td><td className="border-2 border-[#003366] font-black text-blue-800">{q.successRate}%</td></tr>
                                            ))}
                                        </tbody>
                                    </table>
                                    <table className="w-full border-collapse text-[10px] text-center font-bold">
                                        <thead className="bg-[#003366] text-white">
                                            ...
                                            <tr className="bg-blue-50 text-[#003366]"><th></th><th></th><th>العدد</th><th>النسبة</th></tr>
                                        </thead>
                                        <tbody>
                                            <tr className="h-10"><td className="border-2 border-[#003366] text-green-600">مرتفع</td><td className="border-2 border-[#003366] text-right pr-3">لديهم أداء متقدم وحلول إبداعية للمسائل</td><td className="border-2 border-[#003366]">{stats.levels.HIGH.length}</td><td className="border-2 border-[#003366] text-green-700">{Math.round((stats.levels.HIGH.length / stats.studentsList.length) * 100)}%</td></tr>
                                            <tr className="h-10"><td className="border-2 border-[#003366] text-blue-600">متوسط</td><td className="border-2 border-[#003366] text-right pr-3">لديهم تميز نسبي في حل المسائل متوسطة المستوى</td><td className="border-2 border-[#003366]">{stats.levels.MEDIUM.length}</td><td className="border-2 border-[#003366] text-blue-700">{Math.round((stats.levels.MEDIUM.length / stats.studentsList.length) * 100)}%</td></tr>
                                            <tr className="h-10"><td className="border-2 border-[#003366] text-orange-600">منخفض</td><td className="border-2 border-[#003366] text-right pr-3">لديهم خلل في نواتج التعلم وضعف في التأسيس</td><td className="border-2 border-[#003366]">{stats.levels.LOW.length}</td><td className="border-2 border-[#003366] text-orange-700">{Math.round((stats.levels.LOW.length / stats.studentsList.length) * 100)}%</td></tr>
                                            <tr className="h-10"><td className="border-2 border-[#003366] text-red-600">منخفض جداً</td><td className="border-2 border-[#003366] text-right pr-3">يحتاجون إلى تحسين كبير ومتابعة لصيقة</td><td className="border-2 border-[#003366]">{stats.levels.VERY_LOW.length}</td><td className="border-2 border-[#003366] text-red-700">{Math.round((stats.levels.VERY_LOW.length / stats.studentsList.length) * 100)}%</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                                        ...
                                        <div className="h-60 border p-2 bg-white">
                                            ...
                                            <ResponsiveContainer width="100%" height="100%">
                                                <ReBarChart data={stats.studentsList.map((s,i)=>({name:i+1, score:s.pct, level:s.level}))}>
                                                    <CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" tick={{fontSize:8}} /><YAxis domain={[0, 100]} tick={{fontSize:8}} />
                                                    <ReBar dataKey="score">{stats.studentsList.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.level === 'HIGH' ? '#10b981' : entry.level === 'MEDIUM' ? '#3b82f6' : entry.level === 'LOW' ? '#f59e0b' : '#ef4444'} />))}</ReBar>
                                                </ReBarChart>
                                            </ResponsiveContainer>
                                        </div>
                                    </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center border-4 border-dashed border-gray-200 rounded-[3rem] bg-white p-10 text-center">
                    <Upload size={48} className="text-green-600 mb-4"/><h3 className="text-xl font-black text-gray-800 mb-2">حلل استجابات Forms جديدة</h3><p className="text-xs text-gray-400 mb-8 max-w-xs">سيقوم النظام باستخراج المهارات وتصنيف الطلاب آلياً.</p>
                ...
                <div className="flex-1 flex flex-col gap-4 overflow-hidden animate-fade-in">
                    <div className="bg-white p-4 rounded-2xl border shadow-sm flex flex-wrap gap-4 items-center">
                        <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-xl border">
                            <Filter size={16} className="text-gray-400"/><span className="text-xs font-bold text-gray-500">تصفية الصف:</span>
                            <select className="bg-transparent text-sm font-bold outline-none" value={archiveGradeFilter} onChange={e=>setArchiveGradeFilter(e.target.value)}><option value="">الكل</option><option value="الأول">الأول</option><option value="الثاني">الثاني</option><option value="الثالث">الثالث</option></select>
                        </div>
                    </div>
