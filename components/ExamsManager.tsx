...
import { 
    ...
    Library, Copy, BarChart2, CheckCircle, XCircle, Sparkles, Loader2, Wand2, ArrowRight, ShieldCheck, ChevronLeft, Clock, Settings2, Calendar, Video, Globe, AlertCircle
} from 'lucide-react';
                            ...
                            return (
                                <div key={exam.id} className={`bg-white p-6 rounded-[2.5rem] border-2 transition-all group flex flex-col h-80 relative overflow-hidden ${active ? 'border-emerald-500 shadow-emerald-100 shadow-2xl' : 'border-slate-100 hover:border-brand-300 shadow-sm'}`}>
                                    ...
                                    <div className="pt-4 flex gap-2 border-t border-slate-50">
                                        ...
                                        <button className="flex-1 py-2.5 bg-brand-50 text-brand-600 rounded-xl text-[10px] font-black border border-brand-100 flex items-center justify-center gap-1.5 hover:bg-brand-100 transition-all">النتائج</button>
                                    </div>
                                </div>
                            );
                        })}
