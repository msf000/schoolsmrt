...
import { getAssignments, getAcademicTerms, getReportHeaderConfig, getRemedialPlans } from '../services/storageService';
import { generateStudentAnalysis } from '../services/geminiService';
import { 
    FileText, Printer, Search, PieChart as PieChartIcon, Users, 
    TrendingUp, Loader2, Award, Activity, Sparkles, Plus, Calendar, Bot, 
    ArrowRight, CheckCircle, XCircle, Star, MessageCircle, Medal, 
    BrainCircuit, Radar as RadarIcon, History, LifeBuoy, LineChart as LineChartIcon,
    BookOpen, ClipboardList, ListFilter, Target
} from 'lucide-react';
import { 
...
type FollowUpTab = 'SUMMARY' | 'HOMEWORK' | 'ACTIVITY' | 'EXAMS' | 'BEHAVIOR' | 'REMEDIAL' | 'AI';
...
const StudentFollowUp: React.FC<StudentFollowUpProps> = ({ students = [], performance = [], attendance = [], currentUser }) => {
    ...
    const [activeTab, setActiveTab] = useState<FollowUpTab>('SUMMARY');
    ...
    const stats = useMemo(() => {
        ...
        const getCategoryData = (cat: PerformanceCategory) => sPerf.filter(p => p.category === cat);
            ...
            {student && stats ? (
                <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex bg-white rounded-2xl border p-1 mb-6 shrink-0 overflow-x-auto no-scrollbar">
                        ...
                        <TabBtn label="Gemini" icon={<Bot size={16}/>} active={activeTab==='AI'} onClick={()=>setActiveTab('AI')}/>
                    </div>
                            ...
                            <div className="space-y-6 animate-fade-in">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="bg-white p-6 rounded-[2rem] border shadow-sm h-80">
                                        <h3 className="font-black text-gray-800 mb-4 flex items-center gap-2 text-sm"><RadarIcon size={18} className="text-indigo-600"/> رادار المهارات للفترة</h3>
                                    ...
                                    <table className="w-full text-right text-sm">
                                        ...
                                        <tbody className="divide-y">
                                            ...
                                            {(activeTab === 'HOMEWORK' ? stats.homeworks : activeTab === 'ACTIVITY' ? stats.activities : stats.exams).length === 0 && (
                                                <tr><td colSpan={4} className="p-20 text-center text-gray-300 font-bold">لا توجد بيانات مسجلة لهذه الفترة</td></tr>
                                            )}
                                        </tbody>
                                    </table>
                                </div>
                        ...
                        {activeTab === 'BEHAVIOR' && (
                            <div className="space-y-4 animate-fade-in">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between mb-1">
                                                <span className={`font-bold text-sm ${a.behaviorStatus === BehaviorStatus.POSITIVE ? 'text-green-700' : 'text-red-700'}`}>
                                                    {a.behaviorStatus === BehaviorStatus.POSITIVE ? 'سلوك إيجابي / مشاركة' : 'ملاحظة سلوكية / تنبيه'}
                                                </span>
                                ...
                                {stats.sAtt.filter(a => a.behaviorStatus && a.behaviorStatus !== BehaviorStatus.NEUTRAL).length === 0 && (
                                    <div className="p-20 text-center text-gray-300 font-bold bg-white rounded-3xl border">لا توجد ملاحظات سلوكية مسجلة.</div>
                                )}
                            </div>
                        )}
                            ...
                            <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm animate-fade-in flex flex-col min-h-[400px]">
                                <div className="flex justify-between items-center mb-8 border-b pb-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div><h3 className="text-xl font-black text-gray-800">التشخيص التربوي الذكي</h3><p className="text-xs text-gray-400">تحليل معزز بالذكاء الاصطناعي لهذه الفترة</p></div>
                                    </div>
...
const TabBtn = ({ label, icon, active, onClick }: any) => (
    <button onClick={onClick} className={`flex-1 flex items-center justify-center gap-2 py-4 px-2 rounded-2xl font-black text-xs transition-all whitespace-nowrap ${active ? 'bg-indigo-600 text-white shadow-xl scale-105' : 'text-gray-500 hover:bg-gray-50'}`}>{icon} {label}</button>
);
