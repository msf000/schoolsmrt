...
import { Table, Plus, Trash2, Settings, Calendar, X, Check, RefreshCw, Loader2, Zap, CloudLightning, ListFilter, Tag, Printer, CheckCircle, PieChart, Sheet, ArrowUpDown, Link as LinkIcon, Edit3, Target, Layout, ExternalLink, Globe, Save, Layers, barChart as BarChartIcon, TrendingUp } from 'lucide-react';
...
const WorksTracking: React.FC<WorksTrackingProps> = ({ students: initialStudents, performance, attendance, onAddPerformance, currentUser }) => {
    ...
    // الأعمدة المعروضة بناءً على التبويب المختار
    ...
    // الأعمدة داخل نافذة الإعدادات بناءً على الفلتر
    ...
    // حساب أعمال السنة بناءً على الأوزان
    ...
    // Fix: Use Object.entries to correctly infer 'val' as string and find assignment by ID to fix logic and TS errors
    ...
    // Fix: Implement missing handleAddCategory function
    ...
    // Fix: Implement missing handleDeleteCategory function
    const handleDeleteCategory = (id: string) => {
        if (confirm('هل أنت متأكد من حذف هذا التبويب؟ سيتم إخفاء الأعمدة المرتبطة به.')) {
    ...
    const handleFetchSheet = async () => {
        ...
        try {
            ...
            alert('خطأ: تأكد أن ملف قوقل شيت متاح "لأي شخص لديه الرابط"');
        } finally {
        ...
        <div className="p-4 md:p-6 h-full flex flex-col bg-gray-50 animate-fade-in relative overflow-hidden font-tajawal">
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 mb-4 flex flex-col md:flex-row justify-between gap-4 print:hidden">
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <select className="p-2.5 border rounded-xl bg-white text-xs font-black outline-none shadow-sm min-w-[120px] border-gray-200" value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)}>
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => setIsSettingsOpen(true)} className="p-2.5 bg-white text-gray-400 border border-gray-200 rounded-xl hover:text-indigo-600 shadow-sm transition-all"><Settings size={20}/></button>
                </div>
            </div>

            {/* نظام التبويبات والجدول */}
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-200 flex-1 overflow-hidden flex flex-col">
                ...
                <div className="flex-1 overflow-auto custom-scrollbar">
                                    ...
                                    filteredAssignments.map(a => (
                                        <th key={a.id} className="p-3 border-l border-gray-100 min-w-[130px] relative group">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <span className="text-[8px] opacity-50 font-mono mt-0.5">الدرجة: {a.maxScore}</span>
                    ...
                    {(activeTab !== 'YEAR_WORK' && filteredAssignments.length === 0) && (
                        <div className="p-20 text-center text-gray-300 font-black italic">أضف أعمدة رصد من الإعدادات أو اربط ملف قوقل شيت للبدء</div>
                    )}
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col animate-zoom-in">
                        <div className="p-6 border-b bg-gray-50 flex justify-between items-center">
                            <div>
                                <h3 className="font-black text-xl text-gray-800 flex items-center gap-2"><Settings size={22} className="text-indigo-600"/> إعدادات السجل الذكية</h3>
                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">تخصيص الأعمدة، المزامنة، وتوزيع الدرجات</p>
                            </div>
                        ...
                        <div className="flex bg-white border-b overflow-x-auto no-scrollbar shadow-inner px-2">
                            ...
                            <button onClick={() => setSettingsTab('SHEET')} className={`px-8 py-5 font-black text-xs whitespace-nowrap transition-all border-b-4 ${settingsTab === 'SHEET' ? 'text-emerald-600 border-emerald-600 bg-emerald-50/30' : 'text-gray-400 border-transparent'}`}>ربط Google Sheets</button>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 bg-gray-50 custom-scrollbar">
                                ...
                                <div className="space-y-6 animate-fade-in">
                                    <div className="flex flex-wrap items-center justify-between gap-4 mb-2">
                                        <div className="flex items-center gap-3 bg-white p-2 rounded-xl border shadow-sm">
                                            <span className="text-xs font-black text-gray-500 mr-2">تصفية حسب التبويب:</span>
                                            <select className="bg-gray-100 p-1.5 rounded-lg text-xs font-black outline-none" value={settingsCategoryFilter} onChange={e => setSettingsCategoryFilter(e.target.value)}>
                                    ...
                                    <div className="bg-indigo-900 text-white p-6 rounded-3xl shadow-xl flex flex-col md:flex-row gap-4 items-end border-b-8 border-indigo-700">
                                        <div className="flex-1 w-full space-y-3">
                                            <label className="block text-[10px] font-black text-indigo-300 uppercase tracking-widest">إضافة عمود يدوي لـ "{categories.find(c=>c.id===activeTab)?.label}"</label>
                                            ...
                                            <input className="w-full p-2 bg-white/5 border-white/10 text-white rounded-lg text-[10px] outline-none dir-ltr" placeholder="رابط مشاركة خارجي (اختياري)..." value={newCol.url} onChange={e => setNewCol({...newCol, url: e.target.value})}/>
                                        </div>
                                        ...
                                        <table className="w-full text-right text-sm">
                                            <thead className="bg-gray-100/50 text-gray-500 font-black text-[10px] uppercase border-b">
                                                <tr>
                                                    ...
                                                    <th className="p-4">اسم العمود (سحابي)</th>
                            ...
                            {settingsTab === 'WEIGHTS' && (
                                <div className="max-w-xl mx-auto bg-white p-8 rounded-[2.5rem] border shadow-xl animate-fade-in relative overflow-hidden">
                                    ...
                                    <p className="text-xs text-gray-400 mb-6 font-bold">حدد الوزن النسبي لكل قسم ليقوم النظام بحساب المجموع تلقائياً في صفحة "أعمال السنة".</p>
                                    ...
                                    <div className="mt-8 pt-6 border-t flex items-center justify-between">
                                        ...
                                        <button onClick={()=>{localStorage.setItem('works_weights', JSON.stringify(weights)); alert('تم حفظ توزيع الأوزان بنجاح!');}} className="bg-orange-600 text-white px-10 py-3.5 rounded-2xl font-black shadow-lg hover:bg-orange-700 transition-all flex items-center gap-2"><Check size={18}/> اعتماد التوزيع</button>
                                    </div>
                                </div>
                            )}
                            ...
                            {settingsTab === 'SHEET' && (
                                <div className="space-y-6 max-w-3xl mx-auto animate-fade-in">
                                    <div className="bg-emerald-50 p-8 rounded-[2.5rem] border border-emerald-100 shadow-inner">
                                        ...
                                        <p className="mt-3 text-[10px] text-emerald-600 font-bold">تنبيه: يجب أن يكون الملف متاحاً "لأي شخص لديه الرابط" ليتمكن المتابع الذكي من قراءته.</p>
                                    </div>
                                    ...
                                    {sheetNames.length > 0 && (
                                        <div className="bg-white rounded-[2.5rem] border border-gray-200 overflow-hidden shadow-xl animate-slide-up">
                                            <div className="p-5 bg-gray-50 border-b flex gap-4 items-center">
                                                ...
                                                <span className="text-xs font-black text-gray-500">اختر ورقة العمل:</span>
