...
import { Save, CheckCircle, ExternalLink, Loader2, Table, AlertCircle, Link as LinkIcon, Edit2, Cloud, PieChart, Calculator, TrendingUp, Sigma, Activity, Target } from 'lucide-react';
...
const WorksTracking: React.FC<WorksTrackingProps> = ({ students, performance, attendance, onAddPerformance }) => {
    ...
    useEffect(() => {
        ...
        if (masterUrl && !isGenerating) handleAutoSyncForTab(activeTab);
    }, [activeTab, masterUrl]);
    ...
    return (
        <div className="p-6 h-full flex flex-col animate-fade-in relative">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {activeTab !== 'YEAR_WORK' && (
                            <button onClick={handleSaveGrid} className="bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 shadow-md transition-colors font-bold text-sm hover:bg-green-700">
                                {savedSuccess ? <CheckCircle size={18} /> : <Save size={18} />} {savedSuccess ? 'تم الحفظ' : 'حفظ يدوي'}
                            </button>
                        )}
                    </div>
                </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end border-b mb-4 gap-4">
                <div className="flex gap-2">
                    {(['ACTIVITY', 'HOMEWORK', 'PLATFORM_EXAM', 'YEAR_WORK'] as PerformanceCategory[]).map(cat => (
                        <button key={cat} onClick={() => setActiveTab(cat)} className={`px-4 py-2 font-bold text-sm border-b-2 transition-colors relative ${activeTab === cat ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                            <div className="flex items-center gap-2">
            ...
            <div className="flex-1 overflow-auto bg-white rounded-xl shadow border border-gray-200">
                {activeTab === 'YEAR_WORK' ? renderYearWorkTable() : !masterUrl ? (
