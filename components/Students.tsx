...
import { 
    Search, LayoutGrid, List, UserCheck, Activity, Trash2, ArrowRight, Zap, RefreshCw
} from 'lucide-react';
...
const Students: React.FC<StudentsProps> = ({ 
  ...
  const filteredStudents = useMemo(() => {
    return students.filter(s => {
        // حماية ضد القيم الفارغة لضمان عدم اختفاء الطلاب
        const name = s.name || '';
        const nid = s.nationalId || '';
        const sClass = s.className || '';
        
        const matchesSearch = name.toLowerCase().includes(searchTerm.toLowerCase()) || nid.includes(searchTerm);
        const matchesClass = !selectedClass || sClass === selectedClass;
  ...
  const handleQuickAttendance = async (studentId: string, status: AttendanceStatus) => {
    ...
    try {
        ...
        showToast(`تم تسجيل ${status === AttendanceStatus.PRESENT ? 'حضور' : 'غياب'} الطالب بنجاح`, 'SUCCESS');
    } catch (e) {
        showToast('فشل التزامن مع السحابة، سيتم الحفظ محلياً', 'INFO');
    } finally {
        setIsSyncing(false);
    }
  };
  ...
  return (
    <div className="p-6 h-full flex flex-col bg-[#F8FAFC] animate-fade-in font-tajawal overflow-hidden">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8 shrink-0">
        <div>
            <h2 className="text-3xl font-black text-slate-800">إدارة سجل الطلاب</h2>
            <p className="text-sm text-slate-400 font-bold mt-1 uppercase tracking-widest">إجمالي الطلاب المكتشفين سحابياً: {students.length}</p>
        </div>
        <div className="flex items-center gap-3">
            {isSyncing && <div className="flex items-center gap-2 text-indigo-600 bg-indigo-50 px-4 py-2 rounded-xl text-xs font-black animate-pulse"><RefreshCw size={14} className="animate-spin"/> جاري المزامنة...</div>}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-24 pr-1">
        {filteredStudents.length > 0 ? (
                    ...
                    {filteredStudents.map(student => (
                        <div key={student.id} className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2 mt-1">
                                            <span className="text-[10px] font-black text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-full uppercase">{student.className || 'بدون فصل'}</span>
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-6">
                                <button onClick={() => handleQuickAttendance(student.id, AttendanceStatus.PRESENT)} className="flex flex-col items-center justify-center p-3 bg-emerald-50 text-emerald-600 rounded-2xl border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all group/btn">
                                    <UserCheck size={20} className="mb-1 transition-transform group-hover/btn:scale-110"/>
                                    <span className="text-[10px] font-black">تحضير</span>
                                </button>
                                <button onClick={() => navigate('/works', { state: { studentId: student.id } })} className="flex flex-col items-center justify-center p-3 bg-indigo-50 text-indigo-600 rounded-2xl border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all group/btn">
                                    <Activity size={20} className="mb-1 transition-transform group-hover/btn:scale-110"/>
                                    <span className="text-[10px] font-black">رصد أداء</span>
                                </button>
                            </div>

                            <div className="pt-4 border-t border-slate-50 flex justify-between items-center">
                                <button onClick={() => navigate('/followup', { state: { studentId: student.id } })} className="text-[11px] font-black text-indigo-600 hover:underline flex items-center gap-1">
                                    فتح الملف الموحد <ArrowRight size={14}/>
                                </button>
                                <button onClick={() => { if(confirm('حذف الطالب نهائياً من السحابة؟')) onDeleteStudent(student.id); }} className="p-2 text-slate-200 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all"><Trash2 size={16}/></button>
                            </div>
                        </div>
                    ))}
                </div>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {filteredStudents.map(student => (
                                <tr key={student.id} className="hover:bg-indigo-50/20 transition-all group">
                                    <td className="p-5">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center font-black text-indigo-600">{student.name ? student.name.charAt(0) : '؟'}</div>
                                    ...
                                    <td className="p-5">
                                        <div className="flex justify-center gap-2">
                                            <button onClick={() => navigate('/followup', { state: { studentId: student.id } })} className="p-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all">فتح</button>
                                            <button onClick={() => { if(confirm('حذف الطالب؟')) onDeleteStudent(student.id); }} className="p-2 bg-red-50 text-red-500 rounded-xl hover:bg-red-600 hover:text-white transition-all"><Trash2 size={18}/></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
            ...
            <div className="h-full flex flex-col items-center justify-center py-40 text-slate-300 opacity-40">
                ...
                <p className="text-3xl font-black mt-6 uppercase">لا توجد بيانات طلاب معروضة</p>
                <p className="text-sm font-bold mt-2">تأكد من مطابقة مسميات الأعمدة في قاعدة البيانات</p>
            </div>
        )}
      </div>
    </div>
