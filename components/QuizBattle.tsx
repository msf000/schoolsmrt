                ...
                <div className="relative z-10 flex-1 flex flex-col p-10">
                    <div className="flex justify-between items-center mb-10">
                        ...
                        <div className="flex flex-col items-center">
                            <div className={`w-32 h-32 rounded-full border-8 ${timer < 5 ? 'border-red-500 animate-ping' : 'border-white/10'} flex items-center justify-center relative`}>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center gap-12">
                        <div className="bg-white/5 backdrop-blur-3xl p-16 rounded-[4rem] border-4 border-white/10 w-full max-w-5xl text-center relative overflow-hidden shadow-2xl">
                        ...
                        <div className="grid grid-cols-2 gap-6 w-full max-w-5xl">
                            {currentQ.options.map((opt, i) => (
                                <button 
                                    ...
                                    className="p-8 bg-white/10 hover:bg-indigo-600 border-2 border-white/10 rounded-[2.5rem] text-2xl font-black transition-all hover:scale-[1.02] active:scale-95 shadow-xl group"
                                >
                                    <div className="flex items-center gap-6">
                                        <span className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center text-indigo-400 group-hover:bg-white group-hover:text-indigo-600">{String.fromCharCode(65 + i)}</span>
                                        {opt}
                                    </div>
                                </button>
                            ))}
...
const TeamCard = ({ label, color, icon }: any) => (
    <div className={`w-64 h-64 bg-gradient-to-br ${color} rounded-[3.5rem] flex flex-col items-center justify-center gap-4 shadow-2xl border-4 border-white/20 transform rotate-3`}>
...
const ScoreBadge = ({ team, score, active, color }: any) => (
    <div className={`p-8 rounded-[3rem] border-4 transition-all duration-500 flex flex-col items-center gap-2 w-72 ${active ? `${color} border-white shadow-[0_0_40px_rgba(255,255,255,0.2)] scale-110` : 'bg-white/5 border-white/10 opacity-40'}`}>
        <p className="text-xs font-black uppercase tracking-widest">{team}</p>
        <div className="text-6xl font-black">{score}</div>
        {active && <span className="text-[10px] font-black animate-pulse">دور الفريق الآن 🔥</span>}
    </div>
);
