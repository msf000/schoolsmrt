...
import { Bot, Sparkles, Save, MessageSquare, Target } from 'lucide-react';
...
const TeacherAIConfig: React.FC<{ currentUser: SystemUser }> = ({ currentUser }) => {
    ...
    const personalities = [
        { id: 'friendly', name: 'ودود ومحفز', prompt: 'أنت مساعد تعليمي لطيف جداً، تستخدم الرموز التعبيرية بكثرة وتشجع الطلاب بكلمات حماسية سعودية ودية.' },
        { id: 'strict', name: 'رسمي وحازم', prompt: 'أنت مساعد تعليمي رسمي جداً، لغتك العربية فصحى دقيقة، تركز على الحقائق والنتائج الأكاديمية فقط.' },
        { id: 'creative', name: 'إبداعي وعصف ذهني', prompt: 'أنت مساعد تعليمي تركز على التفكير خارج الصندوق، تقترح دائماً أنشطة فنية وحركية ومشاريع مبتكرة للطلاب.' }
    ];
    ...
    return (
        <div className="p-6 h-full flex flex-col bg-gray-50 animate-fade-in font-tajawal overflow-y-auto">
            <div className="max-w-3xl mx-auto w-full space-y-8 pb-10">
                ...
                <div className="bg-white p-8 rounded-[3rem] border shadow-sm space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            {personalities.map(p => (
                                <button 
                                    ...
                                    className={`p-5 rounded-2xl border-2 text-right transition-all ${config.systemInstruction.includes(p.name.split(' ')[0]) || config.systemInstruction === p.prompt ? 'bg-indigo-50 border-indigo-600 shadow-lg' : 'bg-gray-50 border-transparent hover:border-indigo-100'}`}
                                >
                    ...
                    <button 
                        onClick={handleSave}
                        className="w-full py-5 bg-indigo-600 text-white rounded-[2rem] font-black text-lg shadow-xl hover:bg-indigo-700 active:scale-95 transition-all flex justify-center items-center gap-3"
                    >
