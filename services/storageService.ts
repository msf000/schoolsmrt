...
export const authenticateUser = async (id: string, p: string): Promise<SystemUser | null> => {
    // 1. الدخول الافتراضي للإدارة (بدون قاعدة بيانات)
    if (id === 'admin' && p === 'admin') {
        return {
            id: 'admin_root',
            name: 'مدير النظام المتميز',
...
/* Fix: Added updateStudentLearningStyle to handle student learning preference updates in Supabase and localStorage */
...
/* Fix: Added updateSchool to support administrative school updates */
...
/* Fix: Added deleteSchool to support administrative school removal */
...
/* Fix: Added deleteSystemUser to support administrative user removal */
...
export const deleteSubject = (id: string) => {
    const saved = JSON.parse(localStorage.getItem(`local_subjects_global`) || '[]');
    localStorage.setItem(`local_subjects_global`, JSON.stringify(saved.filter((x: any) => x.id !== id)));
};
...
export const deleteAcademicTerm = (id: string) => {
    const saved = JSON.parse(localStorage.getItem(`local_terms_global`) || '[]');
    localStorage.setItem(`local_terms_global`, JSON.stringify(saved.filter((x: any) => x.id !== id)));
};
...
export const deleteTeacherAssignment = (id: string) => {
    const saved = JSON.parse(localStorage.getItem(`local_assignments_map_global`) || '[]');
    localStorage.setItem(`local_assignments_map_global`, JSON.stringify(saved.filter((x: any) => x.id !== id)));
};
...
export const deleteExam = (id: string) => {
    const saved = JSON.parse(localStorage.getItem(`local_exams_global`) || '[]');
    localStorage.setItem(`local_exams_global`, JSON.stringify(saved.filter((x: any) => x.id !== id)));
};
...
/* Fix: Added deleteExamResult to support removal of specific exam attempts */
export const deleteExamResult = async (id: string, eid: string) => {
...
export const deleteFormsDetailedResult = (id: string) => {
    const saved = JSON.parse(localStorage.getItem(`local_forms_results_global`) || '[]');
    localStorage.setItem(`local_forms_results_global`, JSON.stringify(saved.filter((x: any) => x.id !== id)));
};
...
export const deleteQuestionFromBank = (id: string) => {
    const saved = JSON.parse(localStorage.getItem(`local_question_bank_global`) || '[]');
    localStorage.setItem(`local_question_bank_global`, JSON.stringify(saved.filter((x: any) => x.id !== id)));
};
export const getCurriculumUnits = (tid: string): CurriculumUnit[] => JSON.parse(localStorage.getItem(`local_units_${tid}`) || '[]');
...
export const getCurriculumLessons = (uid: string): CurriculumLesson[] => JSON.parse(localStorage.getItem(`local_lessons_${uid}`) || '[]');
...
/* Fix: Added toggleCurriculumLesson to support lesson progress tracking in curriculum manager */
export const toggleCurriculumLesson = async (id: string, completed: boolean, unitId: string) => {
    const current = getCurriculumLessons(unitId);
    const updated = current.map(l => l.id === id ? { ...l, isCompleted: completed, completedAt: completed ? new Date().toISOString() : undefined } : l);
    localStorage.setItem(`local_lessons_${unitId}`, JSON.stringify(updated));
};
...
export const deleteTrackingSheet = (id: string) => {
    const saved = JSON.parse(localStorage.getItem(`local_tracking_sheets_global`) || '[]');
    localStorage.setItem(`local_tracking_sheets_global`, JSON.stringify(saved.filter((x: any) => x.id !== id)));
};
export const getWeeklyPlans = (tid: string): WeeklyPlanItem[] => JSON.parse(localStorage.getItem(`local_weekly_plans_${tid}`) || '[]');

/* Fix: Added saveWeeklyPlanItem to support teacher lesson planning and scheduling */
...
/* Fix: Added Weekly Challenges support for gamification */
...
export const saveChallenge = async (ch: WeeklyChallenge, tid: string) => {
    ...
    localStorage.setItem(`local_challenges_${tid || 'global'}`, JSON.stringify(updated));
};
...
/* Fix: Added Custom Tables support for flexible data storage */
...
export const addCustomTable = async (t: CustomTable) => {
    ...
    const updated = [...current.filter(x => x.id !== t.id), t];
    localStorage.setItem(`local_custom_tables_${t.teacherId || 'global'}`, JSON.stringify(updated));
};
export const deleteCustomTable = async (id: string, tid?: string) => {
    const key = `local_custom_tables_${tid || 'global'}`;
    const current = JSON.parse(localStorage.getItem(key) || '[]');
    localStorage.setItem(key, JSON.stringify(current.filter((x: any) => x.id !== id)));
};
