...
import { 
    ...
    AttendanceStatus, BehaviorStatus
} from '../types';
...
export const fetchAttendance = async (teacherId?: string): Promise<AttendanceRecord[]> => {
    ...
    return (data || []).map((d: any) => ({
        ...
        subject: d.subject, 
        period: d.period, 
        behaviorStatus: d.behavior_status as BehaviorStatus,
...
// Fix: Added missing deleteAssignment function for remote deletion
...
// الدوال المفقودة لضمان عمل التصدير في types
...
export const saveAssignment = async (a: Assignment) => {
    return await supabase.from('assignments').upsert({
        id: a.id, title: a.title, category: a.category, max_score: a.maxScore, is_visible: a.isVisible,
        teacher_id: a.teacherId, term_id: a.termId, period_id: a.periodId, source_metadata: a.sourceMetadata,
...
export const saveAcademicTerm = async (t: AcademicTerm) => {
    return await supabase.from('academic_terms').upsert({
        ...
        is_current: t.isCurrent, teacher_id: t.teacherId,
...
export const saveScheduleItem = async (s: ScheduleItem) => {
    return await supabase.from('schedules').upsert({
        id: s.id, class_id: s.class_id, subject_name: s.subjectName, day: s.day,
        period: s.period, teacher_id: s.teacherId
    });
};
...
export const saveExam = async (e: Exam) => {
    return await supabase.from('exams').upsert({
        ...
        is_active: e.isActive, teacher_id: e.teacherId, date: e.date
    });
};
...
export const addCustomTable = async (t: CustomTable) => {
    return await supabase.from('custom_tables').insert({
        id: t.id, name: t.name, columns: JSON.stringify(t.columns), rows: JSON.stringify(t.rows),
...
export const saveLessonPlan = async (p: StoredLessonPlan) => {
    return await supabase.from('lesson_plans').upsert({
        ...
        content_json: p.contentJson, resources: JSON.stringify(p.resources), 
...
export const saveLessonLink = async (l: LessonLink) => {
    return await supabase.from('lesson_links').insert({
        ...
        grade_level: l.gradeLevel, class_name: l.className
    });
};
...
export const saveFormsDetailedResult = async (r: FormsDetailedResult) => {
    return await supabase.from('forms_results').insert({
        id: r.id, exam_title: r.examTitle, class_name: r.className, date: r.date,
        teacher_id: r.teacherId, questions: JSON.stringify(r.questions),
...
export const getAttendance = (tid?: string): AttendanceRecord[] => JSON.parse(localStorage.getItem(`local_attendance_${tid || 'global'}`) || '[]');
...
// Fix: Added missing getStudents function to provide synchronous local access to students
