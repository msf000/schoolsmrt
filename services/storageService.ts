...
export const fetchAssignments = async (cat?: string, tid?: string): Promise<Assignment[]> => {
    ...
    const { data } = await query;
    return (data || []).map((d: any) => ({
        id: d.id, title: d.title, category: d.category, maxScore: d.max_score, isVisible: d.is_visible,
        teacherId: d.teacher_id, termId: d.term_id, periodId: d.period_id, sourceMetadata: d.source_metadata,
...
/* Fix: Added missing export members for storageService */
...
export const saveScheduleItem = async (s: ScheduleItem) => {
    ...
    return await supabase.from('schedules').upsert({
        id: s.id, class_id: s.classId, subject_name: s.subjectName, day: s.day,
